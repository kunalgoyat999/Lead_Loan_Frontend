...
export const verifyOtpSetPassword = (userId, otp, password) => {
  ...
  return api.post(
    `/api/v1/auth/verify-otp-and-set-password?userId=${userId}&otpCode=${otp}&newPassword=${password}`
  );
};
...
export const getPincodeData = (token, pincode) => {
  return apiWithHeader(token).get(
    `/api/v1/master/postalrecord?pincode=${pincode}`
  );
};

export const getAllLead = (token, page, data) => {
  return apiWithHeader(token).post(
    `/api/lead/v1/getAllLeads?page=${page}&size=12`,
    data
  );
};
...
export const getProfile = (token) => {
  return apiWithHeader(token)
    .get(`/user-service/api/users/v1/fetchProfile`)
    .then((res) => {
